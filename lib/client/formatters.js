...
export function formatLiveTimestamp(isoString, options = {}) {
  const opts = typeof options === 'string' ? { status: options } : (options || {});
  const { status, connectedAt, prefix } = opts;

  if (status === 'disconnected' || status === 'error' || status === 'unauthenticated' || status === 'connecting') {
    if (!isoString) return '—';
    const d = new Date(isoString);
    if (Number.isNaN(d.getTime())) return '—';
    ...
    return `${prefix || 'Last data'} ${timeStr}`;
  }
  ...
  if (isoString) {
    ...
    if (!Number.isNaN(d.getTime())) {
      ...
      return `${prefix || 'Updated'} ${timeStr}`;
    }
  }

  if ((status === 'live' || status === 'idle' || !status) && connectedAt) {
    ...
    if (!Number.isNaN(d.getTime())) {
      ...
      return `${prefix || 'Connected'} ${timeStr}`;
    }
  }
