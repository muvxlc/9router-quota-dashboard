export { getLiveStatusMeta } from './formatters.js';
...
export function formatLiveTimestamp(isoString) {
  if (!isoString) return '—';
...
export function createLiveModelsSubscription(options = {}) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (body && body.authenticated === false) {
          unsubscribed = true;
          cleanupConnection();
          emit({ status: 'unauthenticated', activeModels: [], totalCount: 0 });
          onSessionLoss();
          return;
        }
      }
  ...
  const connect = () => {
    ...
    if (!isVisibleAndOnline()) {
      emit({ status: 'disconnected', activeModels: [], totalCount: 0 });
      return;
    }

    if (!EventSourceClass) {
      emit({ status: 'error', activeModels: [], totalCount: 0 });
      return;
    }
