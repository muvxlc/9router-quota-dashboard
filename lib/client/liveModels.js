...
export function createLiveModelsSubscription(options = {}) {
  ...
  const handleDataPayload = (data) => {
    ...
    if (typeof data.status === 'string') {
      ...
      emit({
        ...
        recentRequests: isDown ? [] : state.recentRequests,
    ...
    const recent = Array.isArray(data.recentRequests) ? data.recentRequests.slice(0, 3) : [];
    emit({
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (body && body.authenticated === false) {
          unsubscribed = true;
          cleanupConnection();
          emit({ status: 'unauthenticated', activeModels: [], totalCount: 0, recentRequests: [], connectedAt: null });
          onSessionLoss();
          return;
        }
      }
  ...
  const connect = () => {
    ...
    if (!isVisibleAndOnline()) {
      emit({ status: 'disconnected', activeModels: [], totalCount: 0, recentRequests: [], connectedAt: null });
      return;
    }

    if (!EventSourceClass) {
      emit({ status: 'error', activeModels: [], totalCount: 0, recentRequests: [], connectedAt: null });
      return;
    }
