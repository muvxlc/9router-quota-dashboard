...
export function getLastKnownGoodQuota(token, connectionId) {
  const key = `${token}:${connectionId}`;
  const entry = quotaCache.get(key);
  if (!entry) return null;
  if (entry.status === 'ok' || (Array.isArray(entry.windows) && entry.windows.length > 0)) {
    return entry;
  }
  return null;
}

export function recordTransientFailure(token, connectionId, lastGood) {
  const key = `${token}:${connectionId}`;
  const existing = quotaCache.get(key) || lastGood;
  const failCount = (existing?.failCount || 0) + 1;
  const backoffMs = Math.min(60000, 5000 * Math.pow(2, failCount - 1));
  const nextRefreshAt = new Date(Date.now() + backoffMs).toISOString();
  const stale = {
    ...lastGood,
    status: 'stale',
    stale: true,
    isStale: true,
    reason: 'STALE_DATA',
    receivedAt: lastGood.receivedAt,
    nextRefreshAt,
    failCount,
  };
  quotaCache.set(key, stale);
  return stale;
}

export function clearCachedQuota(token, connectionId) {
  const key = `${token}:${connectionId}`;
  quotaCache.delete(key);
}

export function allowForceQuotaRefresh(token, connectionId) {
  const key = `${token}:${connectionId}`;
  const now = Date.now();
  if (now - (forceCooldowns.get(key) || 0) < 5000) return false;
  if (forceCooldowns.size >= 500) forceCooldowns.delete(forceCooldowns.keys().next().value);
  forceCooldowns.set(key, now);
  return true;
}

export function setCachedQuota(token, connectionId, quota) {
  if (!quota || quota.status === 'unavailable' || quota.stale || quota.isStale || quota.reason === 'PROVIDER_AUTH_REQUIRED' || quota.reason === 'PROVIDER_UNAVAILABLE') {
    return;
  }
