...
export function sanitizeLiveModels(raw) {
  ...
  if (!raw || typeof raw !== 'object') {
    return { activeModels: [], recentRequests: [], receivedAt };
  }
  ...
  const recentList = Array.isArray(raw.recentRequests) ? raw.recentRequests : [];
  ...
  for (const item of recentList) {
    ...
    const rawStatus = typeof item.status === 'string' ? item.status.replace(/[\x00-\x1f\x7f]/g, '').trim() : '';
    const status = containsCredential(rawStatus) ? 'unknown' : (rawStatus || 'ok').slice(0, 32);

    let timestamp = null;
    if (typeof item.timestamp === 'string' && !containsCredential(item.timestamp)) {
      const parsedDate = new Date(item.timestamp);
      if (!Number.isNaN(parsedDate.getTime())) {
        timestamp = parsedDate.toISOString();
      } else {
        const cleanTs = item.timestamp.replace(/[\x00-\x1f\x7f]/g, '').trim().slice(0, 64);
        if (cleanTs) timestamp = cleanTs;
      }
    }

    const account = sanitizeAccountDisplay(item.account);

    recentRequests.push({ model, provider, status, timestamp, account });
    if (recentRequests.length >= 3) break;
  }
